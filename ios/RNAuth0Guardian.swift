...
class RNAuth0Guardian: NSObject {
    ...
    func device(_ resolve: RCTPromiseResolveBlock, rejecter reject: RCTPromiseRejectBlock){
        if (self.enrolledDevice != nil) {
            let device = [
                ...
                "userId": self.enrolledDevice!.userId,
                "deviceToken": self.enrolledDevice!.deviceToken,
                "notificationToken": self.enrolledDevice!.notificationToken,
                "totp": self.enrolledDevice!.totp?.base32Secret
            ]
            resolve(device)
        } else {
            resolve(false)
    ...
    @objc
    func unenroll(_ resolve: @escaping RCTPromiseResolveBlock, rejecter reject: @escaping RCTPromiseRejectBlock) -> Void {
        Guardian
